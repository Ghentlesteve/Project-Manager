...
Project.getAllTasks().forEach((tasks, index) => {
  tasks.forEach((task) => {
...
taskbox.forEach(column => {
  column.addEventListener("click", event => {
    ...
    if(event.target.classList.contains("delete")){
      ...
      Project.deleteTask(event.target.dataset.id);
    }
  });
